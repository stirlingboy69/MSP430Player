...
)
{
	...
	case FS_FAT16 :
		...
		if (disk_readp(buf, fs->fatbase + (clst>>8), (WORD)(((WORD)(clst &0xFF)) * 2), 2)) break;
		return LD_WORD(buf);
#if _FS_FAT32
	case FS_FAT32 :
...
//assumes fat16 cluster
static CLUST get_fat16_fast
...
)
{
	...
	if (clst < 2 || clst >= FatFs->n_fatent)	/* Range check */
		return 1;

	//if (disk_readp(buf, fs->fatbase + clst / 256, (WORD)(((WORD)clst % 256) * 2), 2)) break;
	if (disk_readp_fast(buf, FatFs->fatbase + (clst>>8), (WORD)( ((WORD)(clst &0xFF)) <<1), 2))
		return 1;	/* An error occured at the disk I/O layer */;
...
)
{
	...
	*br = 0;
	...
	{									/* Repeat until all data transferred */
		if ((fs->fptr&0x1FF /* % 512*/ ) == 0) 
		{				/* On the sector boundary? */
			cs = (BYTE)( (fs->fptr>>9) /* / 512*/ & (fs->csize - 1));	/* Sector offset in the cluster */
			if (!cs) 
		...
		rcnt = (WORD)(512 - (fs->fptr&0x1FF /* % 512*/));		/* Get partial sector data from sector buffer */
...
)
{
	...
	*br = 0;
	...
	while (btr)
	{									/* Repeat until all data transferred */
		if ((FatFs->fptr&0x1FF /* % 512*/ ) == 0)
		{				/* On the sector boundary? */
			cs = (BYTE)( (FatFs->fptr>>9) /* / 512*/ & (FatFs->csize - 1));	/* Sector offset in the cluster */
			if (!cs)
			{								/* On the cluster boundary? */
				clst = (FatFs->fptr == 0) ?			/* On the top of the file? */
						FatFs->org_clust : get_fat16_fast(FatFs->curr_clust);
				if (clst <= 1)
					goto fr_abort;
				FatFs->curr_clust = clst;				/* Update current cluster */
			}
			sect = clust2sect_fast(FatFs->curr_clust);		/* Get current sector */
			if (!sect)
				goto fr_abort;
			FatFs->dsect = sect + cs;
		}
		rcnt = (WORD)(512 - (FatFs->fptr&0x1FF /* % 512*/));		/* Get partial sector data from sector buffer */
		if (rcnt > btr)
			rcnt = btr;
		//dr = disk_readp(!buff ? 0 : rbuff, fs->dsect, (WORD)(fs->fptr % 512), rcnt);
		dr = disk_readp_fast(rbuff, FatFs->dsect, (WORD)(FatFs->fptr & 0x1FF), rcnt);
		if (dr)
			goto fr_abort;
		FatFs->fptr += rcnt;
